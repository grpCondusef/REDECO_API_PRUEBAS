...
const validateDateIsOnTheRangeIni = (fechaHora, errors, calendarioFec) => {
  ...
  if (fechaHoraConvercionFinal < fechaConvercionInicio) {
    errors.push(`Solo es posible eliminar quejas dentro de los 5 primeros días hábiles de presentación.`);
  }
};
...
export const validateFechaRecepcionDelete = async (req, res, next) => {
  try {
    const errors = [];
    ...
    console.error(error);
    return res.status(500).json({ error: "No hay periodo abierto para la eliminación de información." });
  }
};
